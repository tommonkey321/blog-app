 ...
 */
package com.tommonkey.utils.common;
...
public class CfgHttpClientUtil {
    ...
    public static com.tommonkey.utils.http.entity.CfgHttpClient getCfgHttpClientByBusiCode(String busiCode) {
        try {
            com.tommonkey.utils.http.entity.CfgHttpClient rtn = null;
            if(JedisUtils.exists(httpClientCode)) {
                ...
                if (obj != null) {
                    ...
                    if (tempMap.containsKey(busiCode)) {
                        rtn = (com.tommonkey.utils.http.entity.CfgHttpClient) tempMap.get(busiCode);
                    }
                }
            }
            if (rtn == null) {
    ...
    public static com.tommonkey.utils.http.entity.CfgWsClient getCfgWsClientByBusiCode(String busiCode){
        try {
            com.tommonkey.utils.http.entity.CfgWsClient rtn = null;
            if(JedisUtils.exists(wsClientCode)) {
                Object obj = JedisUtils.getObject(wsClientCode);
                if (obj != null) {
                    Map tempMap = (Map<String, Object>) obj;
                    if (tempMap.containsKey(busiCode)) {
                        rtn = (com.tommonkey.utils.http.entity.CfgWsClient) tempMap.get(busiCode);
                    }
                }
            }
            if (rtn == null) {
